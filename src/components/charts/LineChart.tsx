...
import type { SeasonProgressPoint } from '@/lib/types'
import { seasonProgressData as defaultData } from '@/lib/mockData'
  ...
  if (active && payload?.length) {
    ...
    return (
      <div style={{ background: '#1E1E24', border: '1px solid rgba(74,74,255,0.3)', borderRadius: 10, padding: '8px 12px' }}>
        ...
        <p style={{ fontSize: 18, fontWeight: 900, color: '#F5F6FC', margin: '2px 0 0' }}>{score}</p>
...
export default function LineChartComponent({ data, height = 180 }: LineChartProps) {
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={chartData} margin={{ top: 10, right: 10, bottom: 0, left: -20 }}>
        ...
        <XAxis dataKey="match" tick={{ fontSize: 10, fill: '#9DA2B3', fontFamily: 'Inter, sans-serif' }} axisLine={false} tickLine={false} />
        <YAxis domain={[55, 90]} tick={{ fontSize: 10, fill: '#9DA2B3', fontFamily: 'Inter, sans-serif' }} axisLine={false} tickLine={false} />
